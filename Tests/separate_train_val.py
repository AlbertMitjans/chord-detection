...
'''for root, dirs, files in os.walk('C:\\Users\\Albert\\Desktop\\chord-detection\\data\\2'):
    files.sort(key=natural_keys)
    for i, file in enumerate(files):
        if file.endswith('jpg'):
            x = random.random()
        if x < 0.8:
            train.append('data/2/{file}'.format(file=file))
        elif x > 0.8:
            val.append('data/2/{file}'.format(file=file))'''
